(ns indprod.actor
  "IndustrialProductionEngineersActor — the ISCO-08 2141 community indprod actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one indprod operation request
  ...
  The unconditional invariant: the IndustrialProductionEngineersAdvisor can never
  directly commit a record the IndustrialProductionEngineersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [indprod.advisor :as advisor]
            [indprod.governor :as governor]
            [indprod.store :as store]))

(defn build-graph
  "Build a compiled IndustrialProductionEngineersActor graph. `store` implements
  `indprod.store/Store`. `advisor` implements
  `indprod.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :process-id (:process-id proposal)
                                    :payload proposal}]
